...
void find_color(vector<karta>& koloda) {
    int i;
    for(i = 1; i < 36; i++) {
        if ((koloda[i].mast_karta == 1 && koloda[i - 1].mast_karta == 2) || (koloda[i].mast_karta == 0 && koloda[i-1].mast_karta == 3) || (koloda[i].mast_karta == 1 && koloda[i-1].mast_karta == 1) || (koloda[i].mast_karta == 2 && koloda[i-1].mast_karta == 2) || (koloda[i].mast_karta == 3 && koloda[i-1].mast_karta == 3) || (koloda[i].mast_karta == 4 && koloda[i-1].mast_karta == 4)) {
...
void find_dama(vector<karta>& koloda) {
    int i;
    for(i = 0; i < 36; i++) {
        if (koloda[i].real_number == 12 &&
            koloda[i].mast_karta == 3) {
            cout << rang[koloda[i].real_number - 6] << " " << mast[koloda[i].mast_karta] << " находится в колоде на " << i + 1 << "-ом месте" << endl;
            break;
        }
    }
}
...
void print_ace(vector<karta>& t) {
    int i;
    for(i = 0; i < 36; i++) {
        if (t[i].real_number == 14 && t[i].mast_karta ==
            0) {
            cout << rang[t[i].real_number - 6] << " " <<
                 mast[t[i].mast_karta] << " находится в колоде на " << i + 1 << "-ом месте" << endl;
        }
        if (t[i].real_number == 14 && t[i].mast_karta == 1) {
            cout << rang[t[i].real_number - 6] << " " << mast[t[i].mast_karta] << " находится в колоде на " << i + 1 << "-ом месте" << endl;
        }
